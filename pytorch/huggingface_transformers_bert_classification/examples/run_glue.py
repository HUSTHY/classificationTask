...
from transformers_local import glue_compute_metrics as compute_metrics
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, "dev")
...
def predict(args, model, tokenizer, prefix=""):
    ...
    predict_outputs_dirs = (args.output_dir, args.output_dir + "-MM") if args.task_name == "mnli" else (args.output_dir,)

    results = {}
    for predict_task, predict_output_dir in zip(predict_task_names, predict_outputs_dirs):
        predict_dataset = load_and_cache_examples(args, predict_task, tokenizer, "predict")
        ...
        file_path = os.path.join(args.data_dir,'test2.tsv')
        name = []
        label = []
        companys = []
        ...
        result = pd.DataFrame()
        result['name'] = name
        result['company_name'] = companys
        result['predict_label'] = preds
        print(len(result))
        output_predict_file = os.path.join(predict_output_dir, prefix, "companys_product_result.csv")
        result = result[result['predict_label']==1]
        print(len(result))
        result.to_csv(output_predict_file,index=False)
    return results
...
def load_and_cache_examples(args, task, tokenizer, evaluate):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            "dev" if evaluate == 'dev' else "train" if evaluate=="train" else "predict",
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = (
            processor.get_dev_examples(args.data_dir) if evaluate == 'dev' else processor.get_train_examples(args.data_dir)  if evaluate=="train" else processor.get_predict_examples(args.data_dir)
        )
